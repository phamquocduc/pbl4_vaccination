...
export class VaccinationprofileRepository {
    ...
    async create(
        ...
        user: User
    ): Promise<VaccinationProfile | null> {
        const newVaccinationProfile = this.VaccinationprofileRepository.create({
            ...createDto,
            user: { id: user.id },
        });

        return await this.VaccinationprofileRepository.save(
            newVaccinationProfile
        );
    }
    ...
    async getAllProfileById(id: string): Promise<VaccinationProfile[]> {
        const profiles =
            await this.VaccinationprofileRepository.createQueryBuilder(
                'profiles'
            )
