import { Body, Controller, Delete, Get, Param, Post, Put, Request, UploadedFiles, UseFilters, UseInterceptors } from "@nestjs/common";
...
export class UserController{
    ...
    async updateAvatar(@UploadedFiles() file: Express.Multer.File[], @Request() req: any): Promise<User>{
       const userId = req['user'].sub
         console.log(file)
       if(file){
            const uploadPromises = file.map(file => this.cloudinarySevices.uploadFile(file)); 
            const uploadResults = await Promise.all(uploadPromises);

            console.log(uploadResults)
            await this.userSevices.updateAvatar(uploadResults[0].secure_url, userId)
       }
