...
export class UserServices {
    ...
    async updateProfileById(
        ...
        if (!check) {
            throw new CustomAppException(
                createExceptionMessage(ExceptionEnum.PROFILE_NOT_EXIT),
                HttpStatus.BAD_REQUEST
            );
        }

        return await this.vaccinationProfileService.updateProfileById(
            profileId,
            updateDto
        );
    }
    ...
    async deleteProfileById(userId: string, profileId: number): Promise<any> {
        ...
        return await this.vaccinationProfileService.deleteProfileById(
            profileId
        );
    }
    ...
    async changePassword(
        ...
        const { currPassword, newPassword, confirmNewPassword } =
            changePasswordDto;
        ...
        if (!isMatch) {
            throw new CustomAppException(
                createExceptionMessage(ExceptionEnum.CURR_PASSWORD_INVALID),
                HttpStatus.BAD_REQUEST
            );
        }
    ...
    private comparePasswordAndConfirmPassword(
        ...
        if (passWord !== confirmPassWord) {
            throw new CustomAppException(
                createExceptionMessage(
                    ExceptionEnum.CONFIRM_PASSWORD_NOT_MATCH
                ),
                HttpStatus.BAD_REQUEST
            );
        }
