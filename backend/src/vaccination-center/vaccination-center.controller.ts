...
export class VaccinationcenterController{
    ...
    async updateInventory(@Body() updateDto: VaccineInventoryUpdateDto, @Param('id') id: string): Promise<VaccineInventory>{

        const center = await this.vaccinationCenterSevices.getById(Number.parseInt(id))

        return this.vaccineInventorySevices.updateById(center.vaccineinventory.id, updateDto)
    }
